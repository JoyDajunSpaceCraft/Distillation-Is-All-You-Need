...
dataset_train = "csnlp/csnlp_train.jsonl"
dataset_valid = "csnlp/csnlp_val.jsonl"
...
class  MedicalKnowledge(object):
    ...
    def _post_process(self, datasets):
      ...
      def prepare_input(example):
        # ['reason', 'full_text', 'knowledge', 'label'],
        example['input'] = example["full_text"] + " " + example["knowledge"]
        if example["label"] == 0:
          example["label"] = "wrong"
        else:
          example["label"] = "right"
      ...
      datasets = datasets.remove_columns(['full_text', 'knowledge'])
